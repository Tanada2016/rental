...
const OfficerDetail = () => {
  ...
  const officerLoaded = useSelector((state) =>
    state.officers.find((officer) => officer.id === officerId)
  );
  const [isDisabled, setIsDisabled] = useState(true);
  const [officer, setOfficer] = useState(officerLoaded);
  const [checked, setChecked] = useState(
    officer.approved === "true" ? true : false
  );
  ...
  const handleChange = (e) => {
    ...
    } else setOfficer({ ...officer, [e.target.name]: e.target.value });
  };
      ...
      <form className="form-case" onSubmit={handleSubmit} id="form">
        ...
        <div className="checkbox" title="Изменить статус">
          ...
          <label htmlFor="approved">
            {checked ? "Сотрудник одобрен" : "Сотрудник не одобрен"}
          </label>
        </div>
      </form>
    </div>
